	...
	function update(){
		...
		if(!candy.alive){
			...
			do {
				...
				for(var i = 0; i < player.tail; i++){
					// If any part of the body of snake overlap with candy
					if(candy.x == snakeBody[i][0] && candy.y == snakeBody[i][1]){
						collided = true;
						candy.x = Math.floor(Math.random() * gridNum);
						candy.y = Math.floor(Math.random() * gridNum);
						// Once it collide then break from for loop
						break;
					}
				}
			} while(collided); // Collision while loop when it collide
		...
		if(player.tail > 1){
			for(var i = 1; i < player.tail; i++){
				// Head hits the player body
				if(player.x == snakeBody[i][0] && player.y == snakeBody[i][1]){
					player.alive = false;
					// Clear interval stop the game
